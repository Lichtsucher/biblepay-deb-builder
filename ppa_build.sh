...
mkdir -p $BASEDIR/packages/ppa_orig
...
for line in "${releases[@]}"; do
  ...
  docker run -it -e RELEASENAME="$RELEASENAME" -e RELEASEVERSION="$RELEASEVERSION" -e VERSIONSTR="$VERSIONSTR" -v $BASEDIR/packages:/packages -v $HOME/.gnupg:/root/.gnupg -v $BASEDIR/debian:/debian "debpackage-$RELEASENAME" /bin/sh /code/run_ppa_build.sh
